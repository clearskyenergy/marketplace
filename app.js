...
var COL_SAVED_CARTS  = "mkt_saved_carts";
var COL_ALLOWLIST    = "partnerAllowlist"; /* SHARED — used by admin tooling */

/* Approved distributor email domains. Keep this in sync with
   isDistributorDomain() in firestore.rules. Developers register openly;
   only these domains (or an admin) may register as a distributor. */
var DIST_DOMAINS = ["clearsky-usa.com", "rexelusa.com"];

function emailDomain(email) {
  var parts = String(email || "").toLowerCase().split("@");
  return parts.length > 1 ? parts[1] : "";
}
function isDistributorDomain(email) {
  return DIST_DOMAINS.indexOf(emailDomain(email)) > -1;
}
/* map an approved distributor domain to a default distributorId.
   Distributor records are still created by an admin with these ids. */
function distIdForDomain(email) {
  var dom = emailDomain(email);
  if (dom === "rexelusa.com") { return "rexel-energy"; }
  if (dom === "clearsky-usa.com") { return "clearsky"; }
  return null;
}
...
var STATUS_ORDER = ["submitted", "accepted", "quoted", "accepted-quote", "invoiced", "paid", "fulfilled"];
...
function doRegister() {
  ...
  /* Developer registration is OPEN. Distributor role is gated by approved
     email domains (DIST_DOMAINS) — matching isDistributorDomain() in the
     rules. Admin domains also qualify. */
  var allowedDist = isDistributorDomain(email) || isAdminEmail(email);
  if (wantsDist && !allowedDist) {
    $("registerBtn").disabled = false;
    showAuthErr("Distributor accounts are limited to approved partner domains. Register as a developer, or contact info@csebuilders.com to onboard your distribution company.");
    return;
  }

  var role = wantsDist ? "distributor" : "developer";
  var distributorId = wantsDist ? distIdForDomain(email) : null;

  auth.createUserWithEmailAndPassword(email, pass)
    .then(function (cred) {
      return db.collection(COL_PROFILES).doc(cred.user.uid).set({
        name: name, org: org, email: email, emailLower: emailKey,
  ...
  auth.signInWithPopup(provider)
    .then(function (result) {
      ...
      return ref.get().then(function (snap) {
        ...
        /* First-time Google users: distributor role only for approved
           domains; everyone else lands as a developer (open registration). */
        var allowedDist = isDistributorDomain(u.email) || isAdminEmail(u.email);
        var role = allowedDist ? "distributor" : "developer";
        var distributorId = allowedDist ? distIdForDomain(u.email) : null;
        var org = emailDomain(u.email);
        return ref.set({
...
function onAuth(user) {
  ...
  db.collection(COL_PROFILES).doc(user.uid).get().then(function (snap) {
    if (!snap.exists) {
      /* No marketplace profile yet. This happens for (a) a brand-new
         registration whose profile write is still landing, or (b) an
         existing Firebase Auth user (e.g. from another ClearSky portal)
         logging in here for the first time. Retry a couple of times to
         cover (a); if it still isn't there, provision a developer profile
         so (b) can get in — developer registration is open anyway. */
      STATE.profileTries = (STATE.profileTries || 0) + 1;
      if (STATE.profileTries <= 3) {
        setTimeout(function () { onAuth(auth.currentUser); }, 600);
        return;
      }
      return provisionDeveloperProfile(user);
    }
    STATE.profileTries = 0;
    STATE.profile = snap.data();
    STATE.role = resolveRole(STATE.profile);
...
/* Provision a default profile for an authenticated user who has no
   mkt_profiles doc. Approved distributor domains become distributors and
   bind to their distributor id; everyone else becomes a developer. */
function provisionDeveloperProfile(user) {
  STATE.profileTries = 0;
  var email = user.email || "";
  var isDist = isDistributorDomain(email) || isAdminEmail(email);
  var role = isDist ? "distributor" : "developer";
  var distributorId = isDist ? distIdForDomain(email) : null;
  return db.collection(COL_PROFILES).doc(user.uid).set({
    name: user.displayName || email,
    org: emailDomain(email),
    email: email,
    ...
    STATE.profile = { name: user.displayName || email, org: emailDomain(email), email: email, role: role, distributorId: distributorId };
    STATE.role = resolveRole(STATE.profile);
...
/* Single source of truth for a user's effective role. Admin is granted
   ONLY to admin-domain emails — a stored role of 'admin' on any other
   account is ignored. Distributor requires both a stored 'distributor'
   role AND an approved distributor (or admin) domain. Everything else
   is a developer. This prevents role drift / stale-doc privilege bugs. */
function resolveRole(profile) {
  if (!profile) { return "developer"; }
  var email = profile.email || "";
  if (isAdminEmail(email)) { return "admin"; }
  if (profile.role === "distributor" && (isDistributorDomain(email) || isAdminEmail(email))) {
    return "distributor";
  }
  return "developer";
}
...
function routeTo(view) {
  ...
  if (view === "projects") { renderProjectsView(); }
    ...
    if (STATE.role !== "admin") { STATE.view = "projects"; buildNav(); renderProjectsView(); return; }
    renderAdminView();
  }
}
...
function filteredProjects() {
  ...
  if (t === "active") { return p.filter(function (x) { return ["submitted", "accepted", "quoted", "accepted-quote"].indexOf(x.status) > -1; }); }
...
function updateProjTabCounts() {
  ...
  setc("all", p.length);
  setc("active", p.filter(function (x) { return ["submitted", "accepted", "quoted", "accepted-quote"].indexOf(x.status) > -1; }).length);
...
function statusPill(status) {
  ...
  else if (status === "accepted-quote") { cls = "st-accepted"; }
...
function doSubmitProject(seededItems) {
  ...
  for (var i = 0; i < (seededItems || []).length; i++) {
    ...
    cartItems.push({
      ...
      sku: it.product.sku || "",
      catNumber: it.product.catNumber || "",
      upc: it.product.upc || "",
...
function buildActionPanel(p, quote, invoice) {
  ...
  if (canDist) {
    ...
    if (p.status === "quoted") {
      actions.appendChild(el("div", "ap-note", "Quote sent. Waiting for the developer to accept the terms before you invoice."));
    }
    if (p.status === "accepted-quote") {
      actions.appendChild(el("div", "ap-note", "Developer accepted the quote" + (p.poNumber ? " (PO " + esc(p.poNumber) + ")" : "") + ". You're authorized to invoice."));
  ...
  if (isDev) {
    ...
    if (p.status === "quoted") {
      actions.appendChild(el("div", "ap-note", "A quote is ready. Review the quoted unit prices and terms at left, then accept to authorize " + esc(distName(p.targetDistId)) + " to invoice you."));
      var acceptQ = el("button", "btn btn-primary btn-block", "Accept quote & terms");
      acceptQ.style.marginTop = "8px";
      acceptQ.onclick = function () { openAcceptQuoteModal(p, quote); };
      actions.appendChild(acceptQ);
      var decl = el("button", "btn btn-ghost btn-block", "Decline / request changes");
      decl.style.marginTop = "8px";
      decl.onclick = function () {
        var msg = window.prompt("Optional: tell the distributor what needs to change.");
        if (msg === null) { return; }
        addMessage(p._id, "Developer declined the quote" + (msg ? ": " + msg : ".")).then(function () {
          setProjStatus(p, "accepted"); /* back to accepted so distributor can re-quote */
        });
      };
      actions.appendChild(decl);
    }
    if (p.status === "accepted-quote") {
      actions.appendChild(el("div", "ap-note", "You accepted the quote" + (p.poNumber ? " (PO " + esc(p.poNumber) + ")" : "") + ". Waiting for " + esc(distName(p.targetDistId)) + " to issue the invoice."));
    }
    if (p.status === "submitted") {
...
/* ============================================================
   ACCEPT QUOTE / TERMS  (developer authorizes the order + PO)
   ============================================================ */
function openAcceptQuoteModal(p, quote) {
  var wrap = el("div");
  wrap.appendChild(modalHead("Accept quote & terms", esc(p.name)));
  var body = el("div", "modal-body");

  var total = quote && quote.total != null ? quote.total : null;
  var sub = quote && quote.subtotal != null ? quote.subtotal : null;
  var ship = quote && quote.shipping != null ? quote.shipping : 0;
  var tax = quote && quote.tax != null ? quote.tax : 0;

  var summary = '<div class="accept-summary">' +
    (sub != null ? '<div class="ib-row"><span>Materials</span><b>' + money(sub) + '</b></div>' : "") +
    (ship ? '<div class="ib-row"><span>Freight</span><b>' + money(ship) + '</b></div>' : "") +
    (tax ? '<div class="ib-row"><span>Tax</span><b>' + money(tax) + '</b></div>' : "") +
    (total != null ? '<div class="ib-row total"><span>Quote total</span><b>' + money(total) + '</b></div>' : "") +
    '</div>';

  body.innerHTML =
    summary +
    (quote && quote.notes ? '<div class="detail-sec" style="margin-top:14px"><h4>Distributor terms &amp; notes</h4><div class="detail-text">' + esc(quote.notes) + '</div></div>' : "") +
    '<div class="field" style="margin-top:14px"><label>Your PO # (optional)</label><input type="text" id="aq-po" value="' + esc(p.poNumber || "") + '" placeholder="Purchase order number"></div>' +
    '<label class="accept-check"><input type="checkbox" id="aq-agree"> I accept this quote and its terms, and authorize ' + esc(distName(p.targetDistId)) + ' to issue an invoice against this project' + (total != null ? " for " + money(total) : "") + '.</label>';
  wrap.appendChild(body);

  var foot = el("div", "modal-foot");
  var cancel = el("button", "btn btn-ghost", "Cancel"); cancel.onclick = function () { openProjectDetail(p._id); };
  var accept = el("button", "btn btn-primary", "Accept & authorize"); accept.id = "acceptQuoteBtn";
  accept.onclick = function () {
    if (!$("aq-agree").checked) { toast("Please check the box to accept the terms.", true); return; }
    accept.disabled = true; accept.textContent = "Submitting\u2026";
    var po = $("aq-po").value.trim();
    var patch = {
      status: "accepted-quote",
      poNumber: po,
      quoteAcceptedAt: FieldValue.serverTimestamp(),
      quoteAcceptedBy: STATE.profile.name || STATE.profile.email,
      updatedAt: FieldValue.serverTimestamp()
    };
    db.collection(COL_PROJECTS).doc(p._id).update(patch).then(function () {
      return addMessage(p._id, "Developer accepted the quote" + (po ? " (PO " + po + ")" : "") + " and authorized invoicing.");
    }).then(function () {
      toast("Quote accepted. The distributor can now invoice you.");
      openProjectDetail(p._id);
    })["catch"](function (err) {
      accept.disabled = false; accept.textContent = "Accept & authorize";
      toast(friendlyErr(err), true);
    });
  };
...
/* ============================================================
   ...
   ============================================================ */
var CATALOG = { distId: null, products: [], search: "", category: "all", brand: "all", stock: "all" };
...
function renderDistributorPicker() {
  ...
  for (var i = 0; i < STATE.distributors.length; i++) {
    (function (d) {
      ...
      b.onclick = function () { CATALOG.distId = d._id; CATALOG.search = ""; CATALOG.category = "all"; CATALOG.brand = "all"; CATALOG.stock = "all"; renderBrowseCatalog(); };
      card.appendChild(b);
...
/* ---------- developer / admin browse — Rexel-style ---------- */
function renderBrowseCatalog() {
  ...
  loadCatalog(CATALOG.distId).then(function () {
    ...
    head.innerHTML = '<div>' + back + '<h1>' + esc(distName(CATALOG.distId)) + '</h1>' +
      '<p class="ph-sub">' + CATALOG.products.length + ' products \u00b7 filter, add to cart, then submit as a project for a firm quote.</p></div>';
    main.appendChild(head);
    $("catBack").onclick = function () { CATALOG.distId = null; renderCatalogView(); };

    /* search bar spanning the top */
    var searchBar = el("div", "cat-searchbar");
    searchBar.innerHTML =
      '<div class="csb-input"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>' +
      '<input type="text" id="catSearch" placeholder="Search by name, SKU, CAT #, UPC, or brand\u2026" value="' + esc(CATALOG.search) + '"></div>';
    main.appendChild(searchBar);

    /* two-column: filter rail + results */
    var shell = el("div", "cat-shell");
    var rail = el("div", "cat-rail"); rail.id = "catRail";
    var results = el("div", "cat-results"); results.id = "catResults";
    ...
    setTimeout(function () {
      var s = $("catSearch");
      if (s) { s.oninput = function () { CATALOG.search = s.value; renderCatalogResults(); }; }
    }, 0);
...
/* build facet counts from products, honoring the OTHER active filters so
   counts reflect what you'd see if you picked that facet (search always
   applies; a facet doesn't constrain its own counts). */
function facetCounts(dimension) {
  ...
  for (var i = 0; i < CATALOG.products.length; i++) {
    ...
    /* apply search */
    if (q && !productMatchesSearch(p, q)) { continue; }
    /* apply the OTHER two filters, not this dimension */
    if (dimension !== "category" && CATALOG.category !== "all" && p.category !== CATALOG.category) { continue; }
    if (dimension !== "brand" && CATALOG.brand !== "all" && normBrand(p.brand) !== CATALOG.brand) { continue; }
    if (dimension !== "stock" && CATALOG.stock !== "all" && stockKey(p) !== CATALOG.stock) { continue; }

    var key;
    if (dimension === "category") { key = p.category || "misc"; }
    else if (dimension === "brand") { key = normBrand(p.brand); }
    else { key = stockKey(p); }
    if (!key) { continue; }
...
function stockKey(p) {
  ...
  if (st === "in" || (p.stockQty && p.stockQty > 0)) { return "in"; }
  if (st === "out") { return "out"; }
  if (st === "order") { return "order"; }
  return "unknown";
}
var STOCK_LABELS = { in: "In stock", order: "Special order", out: "Out of stock", unknown: "Check availability" };

function productMatchesSearch(p, q) {
  return (p.name || "").toLowerCase().indexOf(q) > -1 ||
         (p.sku || "").toLowerCase().indexOf(q) > -1 ||
         (p.catNumber || "").toLowerCase().indexOf(q) > -1 ||
         (p.upc || "").toLowerCase().indexOf(q) > -1 ||
...
function renderFacetRail() {
  ...
  var anyActive = CATALOG.category !== "all" || CATALOG.brand !== "all" || CATALOG.stock !== "all";
  if (anyActive) {
    var active = el("div", "facet-active");
    var chips = "";
    if (CATALOG.category !== "all") { chips += facetChip("category", CATEGORY_LABELS[CATALOG.category] || CATALOG.category); }
    if (CATALOG.brand !== "all") { chips += facetChip("brand", CATALOG.brand); }
    if (CATALOG.stock !== "all") { chips += facetChip("stock", STOCK_LABELS[CATALOG.stock] || CATALOG.stock); }
    active.innerHTML = '<div class="facet-active-head">Filters<button id="facetClear">Clear all</button></div>' + chips;
    rail.appendChild(active);
    setTimeout(function () {
      var c = $("facetClear");
      if (c) { c.onclick = function () { CATALOG.category = "all"; CATALOG.brand = "all"; CATALOG.stock = "all"; renderFacetRail(); renderCatalogResults(); }; }
      var xs = rail.querySelectorAll("[data-chipdim]");
      for (var k = 0; k < xs.length; k++) {
        xs[k].onclick = function () { CATALOG[this.getAttribute("data-chipdim")] = "all"; renderFacetRail(); renderCatalogResults(); };
      }
    }, 0);
  }

  rail.appendChild(facetGroup("Category", "category", facetCounts("category"), CATEGORY_LABELS));
  rail.appendChild(facetGroup("Brand", "brand", facetCounts("brand"), null));
  rail.appendChild(facetGroup("Availability", "stock", facetCounts("stock"), STOCK_LABELS));
}

function facetChip(dim, label) {
  return '<button class="facet-chip" data-chipdim="' + dim + '">' + esc(label) + ' <span>&times;</span></button>';
}

function facetGroup(title, dim, counts, labelMap) {
  var g = el("div", "facet-group");
  var keys = [];
  for (var k in counts) { if (counts.hasOwnProperty(k)) { keys.push(k); } }
  /* sort: for category use the label map order; else by count desc then name */
  keys.sort(function (a, b) {
    if (counts[b] !== counts[a]) { return counts[b] - counts[a]; }
    var la = labelMap && labelMap[a] ? labelMap[a] : a;
    var lb = labelMap && labelMap[b] ? labelMap[b] : b;
    return la.localeCompare(lb);
  });

  var rows = "";
  var current = CATALOG[dim];
  for (var i = 0; i < keys.length; i++) {
    var key = keys[i];
    var label = labelMap && labelMap[key] ? labelMap[key] : key;
    var sel = (current === key);
    rows += '<button class="facet-opt' + (sel ? " sel" : "") + '" data-dim="' + dim + '" data-key="' + esc(key) + '">' +
      '<span class="fo-label">' + esc(label) + '</span>' +
      '<span class="fo-count">' + counts[key] + '</span></button>';
  }
  if (!rows) { rows = '<div class="facet-empty">\u2014</div>'; }
  g.innerHTML = '<div class="facet-title">' + esc(title) + '</div><div class="facet-opts">' + rows + '</div>';

  setTimeout(function () {
    var btns = g.querySelectorAll(".facet-opt");
    for (var b = 0; b < btns.length; b++) {
      btns[b].onclick = function () {
        var d = this.getAttribute("data-dim"), key = this.getAttribute("data-key");
        CATALOG[d] = (CATALOG[d] === key) ? "all" : key;   /* toggle */
        renderFacetRail();
        renderCatalogResults();
      };
    }
...
function browseFilteredProducts() {
  var q = CATALOG.search.toLowerCase();
  return CATALOG.products.filter(function (p) {
    ...
    if (CATALOG.brand !== "all" && normBrand(p.brand) !== CATALOG.brand) { return false; }
    if (CATALOG.stock !== "all" && stockKey(p) !== CATALOG.stock) { return false; }
    return true;
  });
}

function renderCatalogResults() {
  /* facet counts depend on search, so refresh the rail when search changes */
  renderFacetRail();
  var wrap = $("catResults");
  if (!wrap) { return; }
  wrap.innerHTML = "";

  var items = browseFilteredProducts();
  var bar = el("div", "results-bar");
  bar.innerHTML = '<span class="results-count">' + items.length + ' product' + (items.length === 1 ? "" : "s") + '</span>' +
    '<span class="results-note">Prices are distributor list estimates \u2014 firm price comes on the quote.</span>';
  wrap.appendChild(bar);

  if (!items.length) {
    wrap.appendChild(emptyBox("No products match", "Try clearing a filter or changing your search."));
    return;
  }

  var list = el("div", "prod-list");
  for (var i = 0; i < items.length; i++) { list.appendChild(productRow(items[i])); }
  wrap.appendChild(list);
}

function productRow(p) {
  var rowEl = el("div", "prod-row");

  var img = p.imageUrl
    ? '<div class="pr-img"><img src="' + esc(p.imageUrl) + '" alt=""></div>'
    : '<div class="pr-img noimg"><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.3"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg></div>';

  /* identifiers line: Item # (sku) · CAT # · UPC */
  var ids = [];
  if (p.sku) { ids.push('Item #: <b>' + esc(p.sku) + '</b>'); }
  if (p.catNumber) { ids.push('CAT #: <b>' + esc(p.catNumber) + '</b>'); }
  if (p.upc) { ids.push('UPC: <b>' + esc(p.upc) + '</b>'); }
  var idLine = ids.length ? '<div class="pr-ids">' + ids.join('<span class="pr-dot">&middot;</span>') + '</div>' : "";

  var info = el("div", "pr-info");
  info.innerHTML =
    (p.brand ? '<div class="pr-brand">' + esc(p.brand).toUpperCase() + '</div>' : "") +
    '<div class="pr-name">' + esc(p.name) + '</div>' +
    (p.description ? '<div class="pr-desc">' + esc(p.description) + '</div>' : "") +
    idLine +
    '<div class="pr-cat"><span class="pr-cat-tag">' + esc(CATEGORY_LABELS[p.category] || p.category || "Uncategorized") + '</span></div>';

  var buy = el("div", "pr-buy");
  buy.innerHTML =
    '<div class="pr-price">' + money(p.unitPrice) + ' <small>est. / ' + esc(p.uom || "ea") + '</small></div>' +
    '<div class="pr-stock">' + stockBadge(p) + (p.leadTimeDays ? '<span class="pr-lead">' + p.leadTimeDays + '-day lead</span>' : "") + '</div>';

  var qtyAdd = el("div", "pr-qtyadd");
  qtyAdd.innerHTML =
    '<div class="pr-qty"><button class="qty-btn" data-a="dec">\u2212</button><input type="number" class="pr-qty-in" value="1" min="1"><button class="qty-btn" data-a="inc">+</button></div>';
  var addBtn = el("button", "btn btn-primary btn-sm", "Add to cart");
  qtyAdd.appendChild(addBtn);
  buy.appendChild(qtyAdd);

  setTimeout(function () {
    var input = qtyAdd.querySelector(".pr-qty-in");
    var dec = qtyAdd.querySelector('[data-a="dec"]');
    var inc = qtyAdd.querySelector('[data-a="inc"]');
    if (dec) { dec.onclick = function () { input.value = Math.max(1, (parseInt(input.value, 10) || 1) - 1); }; }
    if (inc) { inc.onclick = function () { input.value = (parseInt(input.value, 10) || 1) + 1; }; }
    addBtn.onclick = function () {
      var qty = Math.max(1, parseInt(input.value, 10) || 1);
      addToCart(CATALOG.distId, p, qty);
      input.value = 1;
    };
  }, 0);
...
function openProductModal(p) {
  ...
  body.innerHTML =
    ...
    '<div class="field-row">' +
      '<div class="field"><label>Item # / SKU</label><input type="text" id="pr-sku" value="' + esc(p ? p.sku : "") + '"></div>' +
      '<div class="field"><label>Brand</label><input type="text" id="pr-brand" value="' + esc(p ? p.brand : "") + '"></div>' +
    '</div>' +
    '<div class="field-row">' +
      '<div class="field"><label>CAT # (catalog number)</label><input type="text" id="pr-cat-num" value="' + esc(p ? (p.catNumber || "") : "") + '"></div>' +
      '<div class="field"><label>UPC</label><input type="text" id="pr-upc" value="' + esc(p ? (p.upc || "") : "") + '"></div>' +
    '</div>' +
    '<div class="field-row">' +
...
function doSaveProduct(existing) {
  ...
  var data = {
    ...
    catNumber: $("pr-cat-num").value.trim(),
    upc: $("pr-upc").value.trim(),
...
/* ============================================================
   CART (developer)
   ============================================================ */
function addToCart(distId, product, qty) {
  ...
  for (var i = 0; i < STATE.cart.length; i++) {
    if (STATE.cart[i].product._id === product._id) { STATE.cart[i].qty += qty; found = true; break; }
  }
  if (!found) { STATE.cart.push({ distId: distId, product: product, qty: qty }); }
  updateCartBadge();
...
function updateCartBadge() {
  ...
  for (var i = 0; i < STATE.cart.length; i++) {
    n += STATE.cart[i].qty;
    total += Number(STATE.cart[i].product.unitPrice || 0) * STATE.cart[i].qty;
  }
  if (badge) {
    badge.textContent = n;
  ...
  if (totalEl) {
    totalEl.textContent = n > 0 ? moneyShort(total) : "";
  }
}
...
function openCartModal() {
  ...
  if (!STATE.cart.length) {
    body.appendChild(emptyBox("Cart is empty", "Browse a distributor's catalog and add products \u2014 or load a saved cart."));
    wrap.appendChild(body);
    var f0 = el("div", "modal-foot");
    var loadEmpty = el("button", "btn btn-ghost", "Load saved cart");
    loadEmpty.onclick = function () { openSavedCartsModal(); };
    var c0 = el("button", "btn btn-primary", "Close"); c0.onclick = closeModal;
    f0.appendChild(loadEmpty); f0.appendChild(c0); wrap.appendChild(f0);
    openModal(wrap, false);
    return;
  }
  ...
  for (var i = 0; i < STATE.cart.length; i++) {
    ...
    rows += '<tr><td><div class="li-name">' + esc(it.product.name) + '</div>' +
      (it.product.sku ? '<div class="li-sku">' + esc(it.product.sku) +
        (it.product.catNumber ? " \u00b7 CAT " + esc(it.product.catNumber) : "") + '</div>' : "") +
      '<div class="li-stock">' + stockBadge(it.product) + '</div></td>' +
  ...
  var foot = el("div", "modal-foot cart-foot");
  var left = el("div", "cart-foot-left");
  var loadBtn = el("button", "btn btn-ghost btn-sm", "Load saved");
  loadBtn.onclick = function () { openSavedCartsModal(); };
  var saveBtn = el("button", "btn btn-ghost btn-sm", "Save cart");
  saveBtn.onclick = function () { saveCurrentCart(); };
  left.appendChild(loadBtn); left.appendChild(saveBtn);

  var right = el("div", "cart-foot-right");
  var clear = el("button", "btn btn-ghost", "Clear");
  ...
  right.appendChild(clear); right.appendChild(submit);
...
/* ============================================================
   SAVED CARTS (developer)
   Stored at mkt_saved_carts/{cartId}. We snapshot the product data so a
   saved cart reloads intact even if the catalog changes later.
   ============================================================ */
function saveCurrentCart() {
  if (!STATE.cart.length) { toast("Nothing to save \u2014 cart is empty.", true); return; }
  var name = window.prompt("Name this cart (e.g. \"Riverside BESS \u2014 gear\"):", "");
  if (name === null) { return; }
  name = (name || "").trim() || ("Cart " + new Date().toLocaleDateString());

  var distId = STATE.cart[0].distId;
  var items = [];
  for (var i = 0; i < STATE.cart.length; i++) {
    var it = STATE.cart[i];
    items.push({
      ...
      stockStatus: it.product.stockStatus || "unknown",
      qty: Number(it.qty || 1)
    });
  }

  db.collection(COL_SAVED_CARTS).add({
    ownerUid: STATE.user.uid,
    name: name,
    distId: distId,
    distName: distName(distId),
    items: items,
    itemCount: items.length,
    createdAt: FieldValue.serverTimestamp(),
    updatedAt: FieldValue.serverTimestamp()
  }).then(function () {
    toast("Cart saved as \u201c" + name + "\u201d.");
  })["catch"](function (err) { toast(friendlyErr(err), true); });
}

function openSavedCartsModal() {
  var wrap = el("div");
  wrap.appendChild(modalHead("Saved carts", "Load a saved cart back in, or delete ones you no longer need."));
  var body = el("div", "modal-body");
  body.innerHTML = '<div class="loading"><div class="spinner"></div></div>';
  wrap.appendChild(body);
  var foot = el("div", "modal-foot");
  var close = el("button", "btn btn-ghost", "Back to cart");
  close.onclick = function () { openCartModal(); };
  foot.appendChild(close);
  wrap.appendChild(foot);
  openModal(wrap, false);

  db.collection(COL_SAVED_CARTS).where("ownerUid", "==", STATE.user.uid).get()
    .then(function (snap) {
      var carts = [];
      snap.forEach(function (d) { var x = d.data(); x._id = d.id; carts.push(x); });
      ...
      body.innerHTML = "";
      if (!carts.length) {
        body.appendChild(emptyBox("No saved carts", "Build a cart from a distributor's catalog and click \u201cSave cart\u201d to keep it for later."));
        return;
      }
      var list = el("div", "saved-cart-list");
      for (var i = 0; i < carts.length; i++) { list.appendChild(savedCartRow(carts[i])); }
      body.appendChild(list);
    })["catch"](function (err) {
      body.innerHTML = "";
      body.appendChild(emptyBox("Could not load", friendlyErr(err)));
    });
}

function savedCartRow(c) {
  var rowEl = el("div", "saved-cart");
  var total = 0;
  for (var i = 0; i < (c.items || []).length; i++) { total += Number(c.items[i].unitPrice || 0) * Number(c.items[i].qty || 1); }

  var info = el("div", "sc-info");
  info.innerHTML = '<div class="sc-name">' + esc(c.name) + '</div>' +
    '<div class="sc-meta">' + esc(c.distName || distName(c.distId)) + ' \u00b7 ' + (c.items ? c.items.length : 0) + ' item' + ((c.items && c.items.length === 1) ? "" : "s") +
    ' \u00b7 ~' + moneyShort(total) + ' \u00b7 saved ' + esc(timeAgo(c.updatedAt || c.createdAt)) + '</div>';

  var acts = el("div", "sc-acts");
  var load = el("button", "btn btn-primary btn-sm", "Load");
  load.onclick = function () { loadSavedCart(c); };
  var del = el("button", "btn btn-danger btn-sm", "Delete");
  del.onclick = function () { deleteSavedCart(c); };
  acts.appendChild(load); acts.appendChild(del);

  rowEl.appendChild(info); rowEl.appendChild(acts);
  return rowEl;
}

function loadSavedCart(c) {
  var replace = true;
  if (STATE.cart.length) {
    replace = window.confirm("Replace your current cart with \u201c" + c.name + "\u201d? (Cancel to merge instead.)");
  }
  var newCart = [];
  for (var i = 0; i < (c.items || []).length; i++) {
    var it = c.items[i];
    /* rebuild a product object from the snapshot */
    var product = {
      _id: it.productId, sku: it.sku, catNumber: it.catNumber, upc: it.upc,
      brand: it.brand, name: it.name, category: it.category,
      unitPrice: it.unitPrice, uom: it.uom, imageUrl: it.imageUrl, stockStatus: it.stockStatus
    };
    newCart.push({ distId: c.distId, product: product, qty: Number(it.qty || 1) });
  }

  if (replace) {
    STATE.cart = newCart;
  } else {
    /* merge only if same distributor, else warn */
    if (STATE.cart.length && STATE.cart[0].distId !== c.distId) {
      toast("That saved cart is from a different distributor \u2014 replace instead.", true);
      return;
    }
    for (var j = 0; j < newCart.length; j++) {
      var merged = false;
      for (var k = 0; k < STATE.cart.length; k++) {
        if (STATE.cart[k].product._id && STATE.cart[k].product._id === newCart[j].product._id) {
          STATE.cart[k].qty += newCart[j].qty; merged = true; break;
        }
      }
      if (!merged) { STATE.cart.push(newCart[j]); }
    }
  }
  updateCartBadge();
  toast("Loaded \u201c" + c.name + "\u201d.");
...
function deleteSavedCart(c) {
  if (!window.confirm("Delete saved cart \u201c" + c.name + "\u201d?")) { return; }
  db.collection(COL_SAVED_CARTS).doc(c._id)["delete"]().then(function () {
